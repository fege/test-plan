...
```bash
uv run python {TESTPLAN_SCRIPTS}/frontmatter.py set {FEATURE_DIR}/TestPlanReview.md auto_revised=true
```

If you could not make any safe edits (for example, source material is missing), set `auto_revised=false`:

```bash
uv run python {TESTPLAN_SCRIPTS}/frontmatter.py set {FEATURE_DIR}/TestPlanReview.md auto_revised=false
```
